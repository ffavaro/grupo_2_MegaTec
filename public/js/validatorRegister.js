window.addEventListener("load", function () {
  ...
  const validator = function (e) {
    ...
    if (fieldValue.length === 0) {
      ...
      field.nextElementSibling.innerText =
        "El nombre debe tener más de dos carácteres";
    } else if (fieldValue.length > 2) {
  ...
  const validatorLastName = function (e) {
    ...
    if (fieldValue.length === 0) {
      ...
      field.classList.add("invalid");
      field.nextElementSibling.innerText =
        "El apellido debe tener más de dos carácteres";
    } else if (fieldValue.length > 2) {
      field.classList.remove("invalid");
      field.nextElementSibling.innerText = "";
    }
  ...
  const validatorPassword = (e) => {
    ...
    if (fieldValue.length === 0) {
      ...
      !regexPassword.test(field.value)
    ) {
      field.classList.add("invalid");
      field.nextElementSibling.innerText =
        "La contraseña debe contener al menos 8 carácteres entre letrás mayúsculas, minúsculas, un número y un carácter especial";
    } else if (fieldValue.length > 2) {
      field.classList.remove("invalid");
      field.nextElementSibling.innerText = "";
    }
  ...
  const validatorImage = (e) => {
    ...
    if (!allowedExt.includes(fieldImage)) {
      field.classList.add("invalid");
      field.nextElementSibling.innerText =
        "Imagen en formato JPG, JPEG, PNG, GIF";
